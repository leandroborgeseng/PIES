...
import knowledgeBase from '../data/hospital_knowledge_base_enriched.json';

const prisma = new PrismaClient();
const kb = knowledgeBase as any;

// ── Helpers ───────────────────────────────────────────────────
function toClassificacao(val: string): string {
  ...
  return map[val] ?? 'RECOMENDADO';
}

function toScalingType(val: string): string {
  const map: Record<string, string> = {
    'PER_BED':    'PER_BED',
    'PER_N_BEDS': 'PER_N_BEDS',
    'PER_SALA':   'PER_SALA',
    'FIXED':      'FIXED',
  };
  return map[val] ?? 'FIXED';
}
  ...
  for (const p of kb.pops_manutencao ?? []) {
    ...
    await prisma.equipmentPOP.upsert({
      ...
      create: {
        ...
        calibracaoTolerancias:         p.calibracao?.tolerancias            ?? null,
